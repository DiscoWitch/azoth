...
if user then
    local controls = user.ControlsComponent
    if controls and controls.mButtonDownThrow and not controls.mButtonDownFire then
        ...
        if mic.mAmountUsed < mic.barrel_size then
            ...
            local speed = 800
            shoot_projectile(self:id(), "mods/azoth/files/items/flasks/flask_vacuum_projectile.xml", x, y,
                speed * math.cos(angle), speed * math.sin(angle), true)
        end
    end
end
