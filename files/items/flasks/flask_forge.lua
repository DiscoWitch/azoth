...
function HasMaterial(item, mat_name, amount)
    ...
    if type(mat_name) == "string" then
        ...
        local count = 0
        for k, v in ipairs(mat_name) do count = count + mats[CellFactory_GetType(v) + 1] end
        return count > amount
    end
end

function CanConvert(fdata) return true end
...
function Convert(fdata)
    ...
    elseif HasMaterial(item, {"azoth_spirit_chaos"}, 500) then
        ...
        local new_flask = Entity(EntityLoad(
                                     "mods/azoth/files/items/flasks/flask_reinforced/flask.xml", x,
                                     y))
...
return {can_convert = CanConvert, convert = Convert}
