...
function newParser()
    ...
    function XmlParser:ToXmlString(value)
        ...
        value = string.gsub(value, "([^%w%&%;%p%\t% ])",
                            function(c) return string.format("&#x%X;", string.byte(c)) end);
        return value;
    end
    ...
    function XmlParser:ParseArgs(node, s)
        string.gsub(s, "([%w_:]+)=([\"'])(.-)%2",
                    function(w, _, a) node:addProperty(w, self:FromXmlString(a)) end)
    end
    ...
    function XmlParser:ParseXmlText(xmlText)
        ...
        if #stack > 1 then error("XmlParser: unclosed " .. stack[#stack]:name()) end
        return top
    end
...
function newNode(name)
    ...
    function node:addChildren(children)
        for i, child in ipairs(children) do self:addChild(child) end
    end
    ...
    function node:toString()
        ...
        output = output .. ">\n"
        for k, child in ipairs(self.___children) do output = output .. child:toString() end
...
function nodeFromDict(name, properties)
    ...
    for k, v in pairs(properties) do new_node:addProperty(k, v) end
    return new_node
end
