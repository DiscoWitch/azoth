...
dofile_once("data/scripts/potions_list.lua")

function spawn_potions(x, y)
    SetRandomSeed(x + 425, y - 243)

    local total_weight = 0
    for k, v in ipairs(potions_list) do
...
function spawn_potion_altar(x, y)
    ...
    if (r > 0.65) then
        LoadPixelScene("data/biome_impl/potion_altar.png", "data/biome_impl/potion_altar_visual.png", x - 5, y - 15, "",
            true)
    end
end
